...
{
    float arkaPlanKonum;
    float mesafe = 10.24f; //ortan noktalarýna göre iþlem yapýyoruz

    // Start is called before the first frame update
    void Start()
    {
        arkaPlanKonum = transform.position.y;
        FindObjectOfType<Gezegenler>().GezegenYerlestir(arkaPlanKonum); // Ýhtiyacýmýz olna y deðeri arka planýn Konum deðeri ile ayný plduðu içi.
    }
